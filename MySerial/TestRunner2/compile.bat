...
REM ========================================
REM TestRunner2 Compile Script
REM ========================================

echo ========================================
echo TestRunner2 Build Script
echo ========================================
echo.
...
if exist Release\TestRunner2.exe (
    echo   %CD%\Release\TestRunner2.exe
    echo.
    echo File size:
    for %%A in (Release\TestRunner2.exe) do echo   %%~zA bytes
) else (
    echo   TestRunner2.exe (check build output for location)
)
echo.
echo ========================================
...
set "SOURCE_FILES=main.cpp ControlClient.cpp ControlServer.cpp Message.cpp ProcessManager.cpp"
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo.
    g++ -std=c++11 %SOURCE_FILES% -o TestRunner2.exe -static-libgcc -static-libstdc++ -O2 -lws2_32
    if %ERRORLEVEL% EQU 0 (
        echo.
        echo ========================================
        ...
        echo ========================================
        echo.
        echo Executable: %CD%\TestRunner2.exe
        if exist TestRunner2.exe (
            for %%A in (TestRunner2.exe) do echo File size: %%~zA bytes
        )
        echo.
        pause
        exit /b 0
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo Compiling: %SOURCE_FILES%
    echo.
    cl /EHsc /std:c++14 /O2 /nologo %SOURCE_FILES% /link /out:TestRunner2.exe ws2_32.lib
    if %ERRORLEVEL% EQU 0 (
        echo.
        echo ========================================
        echo Compilation successful!
...
if exist "%VCVARS_BAT%" (
    ...
    echo cl /EHsc /std:c++14 /O2 /nologo %SOURCE_FILES% /link /out:TestRunner2.exe ws2_32.lib 2^>^&1 >> _temp_compile.bat
